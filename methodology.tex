\todo{introductory para saying that we're going to describe our methodology}
...
Our approach for automatically finding pervasive code patterns aims to find clusters of similar code patterns, using existing implementations of appropriate clustering algorithms. Work in this vein generally makes the assumption that clusters corresponds to classes of change patterns~\cite{hanam2016discovering,campos2019discovering,yang2022mining}. We thus want to associate code changes with datapoints; one of our contributions is a fixed-size embedding which highlights the important parts of a code change.

We now briefly describe our methodology, providing full details in later subsections. Our pipeline for finding classes of pervasive patterns in Rust programs parses program versions, computes differences between them, embeds them in fixed-size datapoints, and then clusters these points.

\todo[inline]{I think we only need to talk about this next bit in a subsection. It's an implementation detail, not fundamental to our approach: ``We decided to implement our pipeline completely in Python. To make this work, we need a way to get an AST as a Python dictionary.''}

\todo[inline]{I wonder if this should really go here, or if there should be a shorter version.}
...
% , encoded in two Python dictionaries; We use dictdiffer to obtain ASTDiffs.
\todo[inline]{Maybe this next paragraph is what you want at the top of 2.1. Then in the next next paragraph you put a shorter description of the pipeline, in a sentence before ``we mined'', and you reference 2.1 there. Anyway think about organization and try to have something that makes sense.}
...
\todo{if you mathtt BCE here, you should do it everywhere, i.e. in the paragraph at the start of 2.1.1.}
...
\todo[inline]{Worse than that, isn't $d$ unbounded?}
...
We propose a novel representation. Similar to the first\todo{which? the naive one?} approach, we order-insensitively define the columns of our dataset as the set of items $\mathtt{NT} \cup \mathtt{BCE}$ we collected from Syn. However, to simulate order-sensitiveness, we add two more steps. First, we record the number of occurrences of items within the paths at each scope. The reason behind this decision is because a different order of program elements yields a different number of occurrences of underlying dimension items \todo{is there an example?}. Second, we semi-automatically design a weighting scheme to prioritize non terminals that we think are more salient for Rust bugs.
...
\todo{that is why what? huh? be more explicit and repeat yourself a bit}
That is why we carried out a manual analysis of clusters. In the manual analysis, we randomly pick 50 clusters; from each cluster we randomly choose 10 datapoints. The person analyzing the cluster \todo{who? be more specific about how we did this} then looks at the code and specifies the clusters that contained more than five datapoints showing similar patterns. For the manual analysis, we used $\epsilon=0.0001, Z=5$ and $\epsilon=0.001, Z=5$, which were the best parameters for $D_g$ and $D_b$, resulting in 577 and 102 clusters, respectively. As a final step, we exclude clusters that contained datapoints from fewer than 3 projects. That is because we wanted our clusters to contain cross-project (e.g. potentially generalizable) patterns as much as possible.  
...
Like Yang et al~\cite{yang2022mining}, and following the definitions of Cotroneo et al.~\cite{cotroneo2019analyzing}, we divide the clusters in three different groups:
...
\todo{can we say more-specific rather than concrete?}
