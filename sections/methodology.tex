...
The output of dictdiff is a list of diffs, where each diff comprises three parts. The first part specifies the modification type:
...
\begin{figure}[h]
    ...
    \caption{\label{fig:essence}Multiplying the number of occurrences of items observed in the tree by their respective weight, resulting in the essence of change (Visualized with a circle pack figure)}
\end{figure}
...
\begin{tabular}{|c|c|c|c|c|}
    ...
    tokio-rs/tokio & 17.2 & 126089 & 3006 & 28.87 \\ \hline
\end{tabular}
\end{table}
...
% cite earlier work that does the search-for-keywords-to-identify-bugs; my earlier EMSE paper has that citation for instance. -- same issue as above, not sure how to cite
...
In this work, we were only interested in the first two groups and ignored the clusters for refactoring changes. Also, we prefer reporting clusters that manifest concrete patterns. An abstract pattern as opposed to a concrete pattern can account for many changes in the programs. For instance, the pattern `Changing a clone of a variable to a borrowing of it in a function argument' is deemed more concrete than `Adding a new statement to a function's body':
