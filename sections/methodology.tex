...
\caption{\label{fig:clustering} Clustering results with nine different combinations of parameters $\epsilon$ and $Z$, respectively separated in columns and rows. In each subplot, the two blue bars in the left indicate the number of clustered datapoints (the number is shown above each bar) after applying clustering on $D_g$ ($G_c$) and $D_b$ ($BC_{c}$), respectively. The two orange bars in the right show similar data but for noise datapoints. In each subplot's legend, $C_G$ and $C_B$ show the number of clusters obtained from $D_g$ and $D_b$, respectively}
\end{figure}
