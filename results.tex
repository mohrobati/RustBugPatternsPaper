...
[discussion on how many of the changes are of each variant: over 90\% are modify]
...
\noindent\textbf{Actionability:} Because attributes have semantic effects, program repair tools can effect repairs by adjusting the set of attributes included. Because so many of our bug fixes change attributes, we believe that IDEs ought to be able to propose relevant attribute changes, as witnessed in our corpus. The example in the text about changing Deserialize to ConfigDeserialize is specific to that program, but other changes would apply more generally. \todo{which?} Also, we demonstrated a case where the developer removed a \texttt{Debug} attribute. A novel type of performance program repair tool working with a run-time profiler could identify unnecessary code and remove attributes that generate it. 
