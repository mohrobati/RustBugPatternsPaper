...
\todo[inline]{Can we justify the 50, both here and in the previous section? ``It was a random number not so large and not too small, how can I justify it?''}
...
\todo[inline]{no, you have to compute the intersection. ``The numbers reported are the intersection'' when is our approach effective and when is it not? ``added above'' more detailed stats would probably be good too ``what kind of stats do you have in mind?''}
...
\end{table}
\todo[inline]{Also do we compare table 2 to what occurs in previous work? ``What previous work?''}
...
We collected 27143 datapoints for borrow-checker related fix patterns. We used $\epsilon=0.001$ and $Z=5$ as our DBSCAN parameters and that resulted in 102 clusters. After manual analysis, as described in~\ref{sec:manual_analysis_cluster_selection}, we ended up with 9 borrow-checker related fix patterns, which we, similar to the previous table, describe in Table~\ref{table:bc}. Here we provide detailed description for each of these patterns:
