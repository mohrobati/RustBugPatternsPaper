...
The human looks at the program output and assigns a rank to it

We carried out a manual experiment for evaluating our code embedding. In the experiment, the human analyzer looks at the provided circlepack figure and the change's source code and assigns a rank to it. This rank reflects the rank of the most important element in the circle pack figure according to the analyzer's view. Based on these ranks, then we calculate Top-$n$ Accuracy for $n=1$ up to $n=5$. Both authors acted as analyzer and carried out the experiment independently. We report the results as the fraction of the changes on which both authors agreed upon its embedding correctness. The final results are shown in Figure~\ref{fig:rq1}.
...
We collected 27143 datapoints for borrow-checker related fix patterns. We used $\epsilon=0.001$ and $Z=5$ as our DBSCAN parameters and that resulted in 102 clusters. After manual analysis, as described in~\ref{sec:manual_analysis_cluster_selection}, we ended up with 8 borrow-checker related fix patterns, which we, similar to the previous table, describe in Table~\ref{table:bc}. Here we provide a detailed description for each of these patterns:
