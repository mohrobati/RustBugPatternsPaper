...
% no, you have to compute the intersection
% when is our approach effective and when is it not?
% more detailed stats would probably be good too
...
We collected 60583 datapoints for general fix patterns using a server with Xeon Gold 5120 Processor (Q3-2017, 14-core 2.2 to 3.2GHz) and the data collection took around 26 hours to complete for both $D_g$ and $D_b$. We used $\epsilon=0.0001$ and $Z=5$ as our DBSCAN parameters and that resulted in 577 clusters. After manual analysis as described in~\ref{sec:manual_analysis_cluster_selection} we ended up with 10 general patterns which we introduce here:
...
% this last sentence immediately prior to here has too many moving parts
...
% maybe discuss the specific example a bit?
...
To give an example, in starship/starship after one of the commits \footnote{\url{https://github.com/starship/starship/commit/56d475578ea508631275772127f49a6949fea6b0}}, the developer borrows the value of \verb+repo_dir+ to keep the ownership of the object in the current scope. Now this object can then be used in another function invocation (\verb+remove_dir_all(repo_dir)+) which fixes a bug. \\
